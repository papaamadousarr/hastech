...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterModule, HttpClientModule, FormsModule, VehicleSearchComponent],
...
export class VehicleModelSelectionComponent implements OnInit {
  ...
  models: { name: string; image_url: string; years?: string }[] = [];
  ...
  goToModel(modelName: string) {
    this.router.navigate(['/vehicle-model', this.selectedBrand.toUpperCase(), modelName]);
  }

  goToModelDetail(modelName: string) {
    this.router.navigate(['/vehicle-model', this.selectedBrand, modelName]);
  }
}