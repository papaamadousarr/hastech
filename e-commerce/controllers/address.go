...
func EditHomeAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var editaddress models.Shipping
		...
		update := bson.D{{Key: "&set", Value: bson.D{primitive.E{Key: "address.0.house_name", Value: editaddress.House}, {Key: "address.0street_name", Value: editaddress.Street}, {Key: "address.0.city_name", Value: editaddress.City}, {Key: "address.0.pincode", Value: editaddress.Pincode}}}}
...
func EditWorkAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if user_id == "" {
			c.Header("Content-Type", "application/json")
			c.JSON(http.StatusFound, gin.H{"Error": "Invalid"})
			c.Abort()
			return
		}
...
func DeleteAddress() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		adresses := make([]models.Shipping, 0)
